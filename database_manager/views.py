...
def update_object(request, pk):
    ...
    if request.method == 'POST':
        ...
        category_map[''] = None
        # Build a per‑category ordering counter
        cat_counters = {name: 0 for name in category_map.keys()}
        for i, col_id in enumerate(col_ids):
            ...
            order_in_cat = cat_counters.get(cat_name, 0)
            cat_counters[cat_name] = order_in_cat + 1
            if col_id == '-1':
                parameter = Parameter(
                    object=obj,
                    name=col_names[i],
...
def add_element_to_object(request, pk):
    ...
    if request.method == 'POST':
        ...
        data_obj.to_pickle(obj.data.path)
        # After saving the row, record any selected child links.  The form
        # includes ``child_link_<object_link_id>`` fields for each linked
        # object.  Depending on the link_type (single vs multiple) we
        # store one or many mappings in ObjectLink_identificators.
        parameters_objects = Object_ParentObject.objects.filter(parent_object=obj)
        for po in parameters_objects:
            if po.link_type == 'multiple':
                # Multi-select: expect multiple values
                child_values = request.POST.getlist(f'child_link_{po.id}')
                # Remove any existing mappings for this parent row and link (avoid duplicates)
                ObjectLink_identificators.objects.filter(object_link=po, parent_object_identificator=new_row_id).delete()
                ...
                # Single select
                child_value = request.POST.get(f'child_link_{po.id}', '')
                if child_value:
                    ObjectLink_identificators.objects.update_or_create(
                        object_link=po,
                        parent_object_identificator=new_row_id,
    ...
    for idx, po in enumerate(parameters_objects):
        ...
        try:
            ...
            if ident_param is not None:
                for _, row in child_df.iterrows():
                    ident_list.append((row['id_to_connect'], row[int(ident_param.id)]))
            parameters_objects_idents.append((po.id, po.object, ident_list, linked_param_ident_id))
        except Exception:
...
def add_objects_links(request, pk):
    ...
    for child_id_str in child_ids:
        ...
        Object_ParentObject.objects.get_or_create(parent_object=parent_object, object=child_obj)
    return HttpResponse('')
